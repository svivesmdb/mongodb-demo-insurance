	...
	public void runConversion() {
		...
		while (doc != null) {
			...
			if (targetMode.equalsIgnoreCase("subsection")) {
				...
				String[] subsections = subsectionNames.split(",");
		...
		try {
			...
			if (row == null) {
				if (hasRows == false) {
					...
					if (row != null) {
						// Iterate over the values and check if we have an ISODate and convert it to java.lang.Date
						for (String key: row.keySet()) {
							Object value = row.get(key);
							if (value != null && value instanceof String && ((String) value).startsWith("ISODate")) {
								String dateToParse = (String) value;
								dateToParse = dateToParse.substring(9, dateToParse.length() - 1);
								SimpleDateFormat ISO8601DATEFORMAT = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
								Date d = ISO8601DATEFORMAT.parse(dateToParse);
								row.put(key, new java.sql.Date(d.getTime()));
							}
						}
					}
